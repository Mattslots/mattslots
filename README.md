...
- Endpoint `/health` compatible Render
...
La V5.1 utilise `src/data/store.json`. Sur l’offre gratuite Render, le disque peut être réinitialisé lors d’un redéploiement. Pour conserver durablement XP, profils et giveaways, utilisez un disque persistant Render ou une base de données dans une version ultérieure.
